...
if(node[:s3fs_fuse][:bluepill])
  ...
  mounted_directories.each do |dir_info|
    mount dir_info[:path] do
      ...
      dump 0
      pass 0
      options "allow_other,uid=www-data,gid=www-data,umask=000,nonempty,url=#{node[:s3fs_fuse][:s3_url]},passwd_file=/etc/passwd-s3fs,retries=20#{",noupload" if dir_info[:no_upload]},#{dir_info[:read_only] ? 'ro' : 'rw'},dev.suid"
      action [:mount, :enable]
